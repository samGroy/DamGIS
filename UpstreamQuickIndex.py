...
BarrierFile = datadir + r"/" + sys.argv[2] #Barriers
#pre-existing barrier watersheds
BasinFile = datadir + r"/" + sys.argv[3] #Barriers

print BarrierFile
Barrier = arcpy.MakeFeatureLayer_management(BarrierFile)
Basin = arcpy.MakeFeatureLayer_management(BasinFile)
...
trigger=1
#counter=0
cur=arcpy.da.SearchCursor(Basin,SitesCols) #just go through ones that previously passed the environment/development/impoundment tests.
for row in cur:
	#Get ID
	ID = int(row[0])
	print '%s' %ID
	...
	#Select current Site
	BasinSelect = arcpy.SelectLayerByAttribute_management(Basin,"NEW_SELECTION",expS)
	...
	UpSelect = arcpy.SelectLayerByLocation_management(Barrier,"INTERSECT",BasinSelect,0,"NEW_SELECTION")
